//
//  _MiscMergeLoopCommandTests.m
//
//  Created by David Aspinall on 12/20/2013.
//  Copyright (c) 2013 Global Village Consulting. All rights reserved.
//

#import <XCTest/XCTest.h>
#import "MiscMergeTests.h"
#import "_MiscMergeLoopCommand.h"

@interface _MiscMergeLoopCommandTests : MiscMergeTests
@end

@implementation _MiscMergeLoopCommandTests
- (void)setUp
{
    [super setUp];
}

- (void)tearDown
{
    [super tearDown];
}

- (void)test_handleEndLoopInTemplate
{
}

// ## END MARKER

@end

